...
export class ReservasService {
  ...
  async listTypeAvailability(params: TypeAvailabilityListParams): Promise<TypeAvailabilityItem[]> { return listTypeAvailabilityUsecase({ rooms: this.roomRepo, reservations: this.repo, blocks: this.blockRepo }, params) } // REQ-HAC-05 (#260): disponibilidad por tipo para el wizard; MISMO port que `createReservation` usa para `type_sold_out` (rooms + reservations + blocks)

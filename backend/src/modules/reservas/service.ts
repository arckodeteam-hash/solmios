...
export class ReservasService {
  ...
  async retryRefund(id: string, currentUser: { id: string; role: string; hotelId?: string }): Promise<RetryRefundResult> { return retryRefundUsecase({ repo: this.repo, auth: this.auth, port: this.orchestrationDeps.retryWebRefund }, id, currentUser) } // #272 — POST /api/reservas/:id/retry-refund: reintenta en Stripe vía el puerto del connector bookingengine-refunds; ownership post-findById en usecases/retry-refund.ts
  async setRefundState(id: string, patch: Record<string, unknown>): Promise<ReservasDTO | null> { return this.repo.update(id, refundStatePatch(patch) as any) } // #272 — escritura acotada de refundStatus/refundedAt/refundPaymentId (sin state machine): la usa bookingengine-refunds, que ya validó reserva y hotel en shared/usecases/web-booking-refund

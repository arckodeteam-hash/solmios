...
const conectados = (...types: string[]) => async () => types.map((type) => ({ type, conectado: true }))

describe('listOverrideChannels', () => {
  it('devuelve el canal conectado que tiene tarifa propia', async () => {
    const out = await listOverrideChannels(rates('', 'OpenChannel'), 'h1', conectados('OpenChannel'))
    expect(out).toEqual(['OpenChannel'])
  })

  it('descarta las filas de un canal que NO está conectado: si no, pisa al que sí vende', async () => {
    const out = await listOverrideChannels(
      rates('', 'OpenChannel', '0a70f83f-3a3a-41de-84c5-ab2fecc8b80c'), 'h1', conectados('OpenChannel'),
    )
    expect(out).toEqual(['OpenChannel'])
  })

  it('un canal conectado SIN tarifa propia no se publica aparte: no hay nada que publicar', async () => {
    const out = await listOverrideChannels(rates('', 'OpenChannel'), 'h1', conectados('OpenChannel', 'booking'))
    expect(out).toEqual(['OpenChannel'])
  })

  it('un canal desconectado queda afuera aunque tenga tarifas', async () => {
    const out = await listOverrideChannels(
      rates('booking'), 'h1', async () => [{ type: 'booking', conectado: false }],
    )
    expect(out).toEqual([])
  })

  it('hotel sin tarifas por canal → vacío, y el full sync sigue siendo de 2 llamadas', async () => {
    const out = await listOverrideChannels(rates('', '', null, undefined), 'h1', conectados('OpenChannel'))
    expect(out).toEqual([])
  })

  it('sin filas de tarifas no se pregunta por los canales conectados', async () => {
    let asked = false
  ...
  it('no duplica un canal que aparece en varias filas', async () => {
    const out = await listOverrideChannels(rates('OpenChannel', 'OpenChannel'), 'h1', conectados('OpenChannel'))
    expect(out).toEqual(['OpenChannel'])
  })
})

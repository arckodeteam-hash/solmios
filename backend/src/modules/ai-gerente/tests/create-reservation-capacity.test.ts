...
describe('create_reservation (Gerente IA) — capacidad (auditoría de integridad, cierre)', () => {
  ...
  it('persiste roomType = rooms.type de la unidad (HAC-01)', async () => {
    const r = repos()
    const rows: any[] = []
    r.reservationRepo.create = async (data: any) => { rows.push(data); return { id: 'res-new', ...data } }
    await executeManagerTool('create_reservation', { roomId: 'room-1', checkIn: '2026-07-20', checkOut: '2026-07-22', adults: 2, guestName: 'Ana' }, HOTEL, r)

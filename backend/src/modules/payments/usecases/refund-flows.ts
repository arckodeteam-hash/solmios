...
export async function refundStripe(deps: RefundFlowDeps, paymentId: string, amount: number | undefined, user: Actor, reason?: string): Promise<PaymentDTO> {
  return settle(deps, await refundViaStripe(deps, paymentId, amount, user, reason), amount, user)
}

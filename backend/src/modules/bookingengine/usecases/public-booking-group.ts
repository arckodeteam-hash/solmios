...
import { normalizeChildAmenityIds, resolveChildAmenityLines, mealPlanNote, mealPlanAddonInput, normalizeIdempotencyKey, resolvePaymentDeadlineAt, isUniqueViolation } from './public-booking'
    ...
    await orm.transaction(async (tx: any) => {
      ...
      if (leader) {
        ...
        const addonRows = buildBookingEngineAddons({
          ...
          mealPlans: resolvedLines.flatMap((l) => (l.mealPlan ? [mealPlanAddonInput(l.mealPlan, l.roomIds.length)] : [])),
        })

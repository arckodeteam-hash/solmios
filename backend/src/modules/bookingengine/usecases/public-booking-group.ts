...
// `Reservations` POR CADA unidad pedida (una por `quantity` de cada línea), todas con el mismo
// `groupId`. Cero migración nueva, cero tocar folios/check-in/housekeeping/planning — todos
// siguen viendo reservas individuales, exactamente como ya sabían operar.
//
...
//
// ─── REQ-HAC-05 (#260) — Cada fila nace POR TIPO, sin unidad (`roomId: null`) ───────────────
// Mismo criterio que `createPublicBookingDirect` (ver la cabecera de public-booking.ts): el
// grupo NO elige habitaciones. Cada línea `{roomType, quantity}` produce `quantity` filas con
// `roomType` y `roomId: null`; la unidad la asigna recepción después (`assign-room.ts`).
//   - Venta: SOLO `countAvailableOfType` (`rooms − booked` por noche, contando reservas del tipo
//     asignadas O sin asignar, más bloqueos). Dos líneas del MISMO tipo en el mismo POST suman:
//     la segunda descuenta lo que ya reclamó la primera (`claimedByType`).
//   - Capacidad: la línea entra si entra en alguna unidad vendible del tipo (la política
//     `room_type_capacity` del hotel, si existe, pisa los campos de la unidad), y no se venden
//     más unidades "grandes" que las que físicamente existen para esa composición.
//   - Precio por unidad: la tarifa del tipo (`sumStayPriceForComposition`) con fallback al
//     MÍNIMO `basePrice` entre las vendibles (`roomTypeProfile`), lo que `/rates` publica.
//   - Amenidades (#290) y cuna (#292) por línea: contra la UNIÓN de `RoomAmenities` de las
//     unidades vendibles del tipo; todas las filas de la línea llevan el mismo snapshot y el
//     mismo `cribUnavailable`.
//
// ─── Anti-overbooking del grupo COMPLETO ─────────────────────────────────────────────────────
// Mismo patrón que `createPublicBookingDirect` (UPDATE sobre `Rooms {hotelId, type}` que toma el
// lock de fila + re-chequeo `availableOfType` DENTRO de la tx) pero aplicado a TODOS los tipos
// del grupo dentro de LA MISMA transacción: si un solo tipo ya no tiene lugar para lo que pide
// el grupo, se aborta el grupo ENTERO (todo o nada) — no puede quedar una reserva a medias.
...
export interface RoomLineInput {
   ...
   *  server-side contra los bebés de ESTA línea Y que alguna unidad vendible del TIPO ofrezca
   *  `custom:cuna` (`CRIB_AMENITY_KEY`), igual que el flujo de 1 habitación (REQ-HAC-05: la fila
   *  nace sin unidad). "Sí" fuerza esa key en `roomAmenities` de la línea (precio de la unión
   *  del tipo) y `needsCrib` se persiste en TODAS las filas de la línea como espejo exacto de su
   *  línea de cuna; "No" la quita aunque venga en el body.
   ...
   *  línea llevan el mismo snapshot. `custom:cuna` se filtra acá (la gobierna `needsCrib`). */
   ...
   *  cada fila de la línea lo lleva (total de la línea = unitario × quantity). */
  mealPlan?: string
}

/** REQ-HAC-05 — el TIPO se agotó entre el `countAvailableOfType` de afuera y el insert (mismo
 *  centinela que `RoomTypeSoldOutConcurrentlyError` en public-booking.ts, con el tipo para el 409). */
class RoomTypeSoldOutConcurrentlyError extends Error {
  constructor(public readonly roomType: string) { super('room_type_sold_out_concurrently'); this.name = 'RoomTypeSoldOutConcurrentlyError' }
}
...
function normalizeRoomLines(raw: any): RoomLineInput[] | null {
  ...
  for (const r of raw) {
    ...
    // la composición de CADA línea y las unidades vendibles de su tipo.
 ...
 * sola operación atómica: todas las filas se crean, o ninguna. Mismo contrato de robustez
 * que `createPublicBookingDirect`: si Stripe falla, el grupo queda creado igual (pending).
 *
 * @param pushAvailability (Legado) Push POR UNIDAD. Desde REQ-HAC-05 las filas nacen sin unidad,
 *                       así que acá NO se invoca; se conserva en la firma para no romper a los
 *                       callers que lo pasan posicionalmente (misma decisión que public-booking.ts).
 * @param pushAvailabilityByType REQ-HAC-05 — push de disponibilidad a las OTAs POR TIPO, una vez
 *                       por tipo distinto del grupo (`canales.pushAvailabilityByRoomType`).
 */
export async function createPublicBookingGroup(
  ...
  // ─── Resolver TODAS las líneas ANTES de la tx: cuántas unidades del tipo, a qué precio ───
  // REQ-HAC-05: no se eligen unidades físicas — sólo se cuenta que el TIPO tenga lugar.
  // REQ-HAC-02 (#257) — unidades del TIPO (en minúsculas) ya reclamadas por líneas anteriores de
  // este POST (2 líneas del mismo roomType con distinta ocupación, por ejemplo "Deluxe para 2 ×1
  ...
  interface ResolvedLine {
    ...
    // propio bebé. #292 — `needsCrib` = la línea de cuna quedó en el snapshot de la línea
    // (resuelto contra la unión del tipo); TODAS las filas de la línea lo persisten igual.
    needsCrib: boolean
    /** Revisión #292 — la línea pidió cuna y NINGUNA unidad vendible del tipo la ofrece (todas
     *  sus filas persisten `cribUnavailable`, va a la nota del grupo y a la respuesta). */
    ...
    // REQ-01 (#290) — snapshot de amenidades de habitación de la LÍNEA (quantity 1, precio de la
    // unión de las unidades vendibles del tipo, cuna incluida cuando `needsCrib`, #292): cada
    // fila persiste el mismo snapshot; `roomAmenitiesTotal` es la Σ de la línea (× `units`).
    roomAmenities: RoomAmenityLine[]
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    // de mirar el inventario del tipo; el mensaje nombra la línea para que el huésped sepa cuál corregir.
    ...
    // REQ-HAC-05 — capacidad SIN elegir unidad: la línea entra si entra en alguna unidad vendible
    // del tipo (recepción elige cuál al asignar), y no se venden más unidades "grandes" que las
    // que físicamente existen para ESTA composición (pedir 2 "para 4" con una sola unidad de 4 no
    ...
    if (fittingUnits < line.quantity) {
      ...
      return {
        status: 409,
        body: {
          error: `Solo hay ${availableForLine} habitación(es) de "${line.roomType}" con capacidad para ${totalGuestsForLine} huésped(es) disponibles para esas fechas (pediste ${line.quantity})`,
    ...
    // ESTA línea, se fuerza "no pedida" sin importar el body. La segunda mitad del gate — ¿alguna
    // unidad vendible del TIPO ofrece `custom:cuna`? — se cierra DESPUÉS de
    // `resolveRoomAmenityLines` contra la unión del tipo. Sí/No únicamente — `cribCount` es 1/0
    // espejo de `needsCrib`, nunca una cantidad elegible.
    ...
    let lineRoomAmenities: RoomAmenityLine[] = []
    let lineRoomAmenitiesUnitTotal = 0
    if (lineRoomAmenityKeys.length > 0) {
      const lineAmenitiesByRoom = await loadRoomAmenitiesFor(orm, typeAvail.sellableRooms.map((r: any) => r.id))
      const resolved = resolveRoomAmenityLines(unionRoomAmenities(lineAmenitiesByRoom), lineRoomAmenityKeys, 1, logger)
      lineRoomAmenities = resolved.lines
      lineRoomAmenitiesUnitTotal = resolved.total
    }
    // #292 — `needsCrib` definitivo de la línea: la línea de cuna quedó (o no) en el snapshot
    // (`hasCribLine`), igual que en public-booking.ts. Si se pidió y NINGUNA unidad del tipo la
    // ofrece, todas las filas de la línea se crean sin cuna con un warn claro.
    const lineNeedsCrib = lineCribRequested && hasCribLine(lineRoomAmenities)
    // Revisión #292 — cuna pedida que el tipo no puede cumplir: cada fila de la línea persiste
    // `cribUnavailable`, la nota del grupo lo dice y la respuesta lo expone.
    const lineCribUnavailable = lineCribRequested && !lineNeedsCrib
    if (lineCribUnavailable) {
      logger?.warn('createPublicBookingGroup: cuna pedida pero ninguna unidad del tipo la ofrece — se crea sin cuna', {
        hotelId, roomType: line.roomType,
      })
    }
    ...
    resolvedLines.push({
      ...
      units: line.quantity, perUnitPrice,
      // MR-10 (#275) — personas de la línea que consumen upsells (adultos + niños con plaza +
      // niños libres, SIN bebés), por unidad; se multiplica por `units` más abajo.
      ...
      cribUnavailable: lineCribUnavailable,
      childrenRatePercentApplied: lineChildrenRatePercentApplied,
      roomAmenities: lineRoomAmenities, roomAmenitiesTotal: round2(lineRoomAmenitiesUnitTotal * line.quantity),
      mealPlan: lineMealPlan, mealPlanTotal: lineMealPlanTotal,
    })
  }

  const roomSubtotal = round2(
    resolvedLines.reduce((s, l) => s + l.perUnitPrice * l.units, 0),
  )
  // REQ-01 (#290) — Σ de las amenidades de habitación de todas las líneas (cada una ya × units).
  ...
  // que `/booking`). El contexto es el CARRITO entero: `rooms` = Σ `units` de todas las
  ...
  if (upsellItems.length > 0 && hotelUpsells) {
    const upsellRooms = resolvedLines.reduce((s, l) => s + l.units, 0)
    const upsellPersons = resolvedLines.reduce((s, l) => s + l.upsellPersonsPerUnit * l.units, 0)
  ...
  const roomsSummary = resolvedLines.map((l) => `${l.roomType}×${l.units} (para ${l.adults})`).join(', ')
  ...
  // REQ-01 (#290) — vistazo rápido por tipo (Σ de la línea = unitario × units); el snapshot
  // unitario vive en `roomAmenities` de CADA fila. La cuna (#292) aparece acá con su precio.
  const roomAmenitiesSummary = resolvedLines
    .filter((l) => l.roomAmenities.length > 0)
    .map((l) => `${l.roomType}: ${l.roomAmenities.map((a) => `${a.name}=${(a.total * l.units).toFixed(2)}`).join(', ')}`)
  ...
  // (2026-09-09) — se lista qué tipos la RECIBIERON (línea con cuna resuelta), sin cantidad.
  ...
  // Revisión #292 — tipos que pidieron cuna y ninguna unidad la ofrece (misma nota que el flujo
  // de 1 habitación, con el tipo para que recepción sepa cuál).
  const cribUnavailableTypes = resolvedLines.filter((l) => l.cribUnavailable).map((l) => l.roomType)
  ...
  // REQ-HAC-05 — unidades pedidas por TIPO (Σ de las líneas de ese tipo, tal como lo mandó el
  // widget): es lo que se lockea y re-chequea en la tx y lo que se empuja a las OTAs al final.
  const unitsByType = new Map<string, number>()
  for (const l of resolvedLines) unitsByType.set(l.roomType, (unitsByType.get(l.roomType) ?? 0) + l.units)

  try {
    await orm.transaction(async (tx: any) => {
      // Anti-overbooking del GRUPO COMPLETO (por TIPO desde REQ-HAC-05): lock + re-chequeo de
      // CADA tipo del grupo. El UPDATE sobre `Rooms {hotelId, type}` serializa a dos compradores
      // del mismo tipo (lock de fila en Postgres READ COMMITTED); quien decide es el RE-CHEQUEO
      // `availableOfType` con el lock tomado, contra el TOTAL que pide el grupo de ese tipo (Σ de
      // sus líneas). Si un solo tipo ya no entra, se aborta el grupo entero (todo o nada) — mismo
      // patrón que `createPublicBookingDirect`, aplicado a N tipos en la misma tx. Sin
      // `updateMany`/`findMany` en la tx (mocks viejos) se degrada, como allá.
      for (const [roomType, unitsOfType] of unitsByType) {
        if (typeof tx.updateMany === 'function') {
          await tx.updateMany('Rooms', { hotelId, type: roomType }, { updatedAt: new Date().toISOString() }).catch(() => 0)
        }
        if (typeof tx.findMany === 'function') {
          ...
          if (fresh.available < unitsOfType) throw new RoomTypeSoldOutConcurrentlyError(roomType)
        }
      }
      ...
      for (const line of resolvedLines) {
        for (let unit = 0; unit < line.units; unit++) {
          ...
          const reservation = await tx.create('Reservations', {
            ...
            id: crypto.randomUUID(), hotelId, roomId: null, roomType: line.roomType, guestId: guest.id, groupId: group.id,
            ...
            // — gateado por línea (bebé + pedida) y cerrado contra la unión del TIPO: `needsCrib`
            // es true si y sólo si `roomAmenities` de la línea trae `custom:cuna` (todas las filas
            // de la línea igual, REQ-HAC-05); `cribCount` su espejo 1/0.
            needsCrib: line.needsCrib, cribCount: line.needsCrib ? 1 : 0,
            // Revisión #292 — la línea pidió cuna y ninguna unidad del tipo la ofrece.
            cribUnavailable: line.cribUnavailable,
            ...
            childAmenities: [],
            childAmenitiesTotal: 0,
            // REQ-01 (#290) — snapshot UNITARIO de la línea (precio de la unión del tipo).
            roomAmenities: line.roomAmenities,
            roomAmenitiesTotal: round2(line.roomAmenities.reduce((s, a) => s + a.total, 0)),
            ...
            // `notes`, que no cambia). Van en TODAS las filas del grupo: cada habitación
      ...
      // habitación, una fila por UNIDAD pedida (el snapshot unitario de la línea × `units`, cuna
      // incluida). Las hermanas no reciben addons — el cobro es uno solo y así se postea al
      // folio una sola vez.
      // Misma tx que las reservas; `notes`/`priceBreakdown` no cambian.
      const leader = reservations[0]
      if (leader) {
        // MR-03 (#268) — una fila `meal_plan` por LÍNEA con régimen, con SU total (persons de la
        // línea × noches × su quantity): Σ filas = `mealPlanTotal` del grupo.
        const addonRows = buildBookingEngineAddons({
          reservationId: leader.id, hotelId, taxRate: totalTaxRateOf(hotelTaxes),
          upsells: upsellLines,
          roomAmenities: resolvedLines.flatMap((l) => Array.from({ length: l.units }, () => l.roomAmenities).flat()),
          mealPlans: resolvedLines.flatMap((l) => (l.mealPlan ? [mealPlanAddonInput(l.mealPlan, l.units)] : [])),
        })
    ...
    if (e instanceof RoomTypeSoldOutConcurrentlyError) {
      logger?.warn(`Grupo abortado: "${e.roomType}" se agotó concurrentemente`, { hotelId, roomType: e.roomType })
  ...
  // REQ-HAC-05 — sin unidad no hay push por habitación: se empuja cada TIPO vendido, una vez.
  for (const roomType of unitsByType.keys()) pushAvailabilityByType?.(hotelId, roomType)
  ...
  return groupResponse(201, group, reservations, lead, sharedAccessToken, guest, checkoutUrl, totalBreakdown, paymentError)
}
...
async function replayPublicBookingGroup(
  ...
  // REQ-HAC-05 — las filas nuevas traen `roomType`; una fila vieja con unidad asignada y sin tipo
  // (anterior al backfill de HAC-01) lo deriva de `Rooms` para que el replay responda lo mismo.
  const withType = await fillRoomTypeFromRooms(orm, hotelId, reservations)
  ...
  return groupResponse(200, group, withType, lead, lead.accessToken, guest, checkoutUrl, totalBreakdown, paymentError, true)
}

/** Replay (#266) — completa `roomType` desde `Rooms` sólo para las filas que no lo traen. */
async function fillRoomTypeFromRooms(orm: any, hotelId: string, reservations: any[]): Promise<any[]> {
  if (reservations.every((r) => r.roomType || !r.roomId)) return reservations
  const rooms: any[] = (await Promise.resolve(orm.findMany?.('Rooms', { hotelId })).catch(() => [])) ?? []
  return reservations.map((r) => (r.roomType || !r.roomId ? r : { ...r, roomType: rooms.find((x: any) => x.id === r.roomId)?.type ?? null }))
}
...
function groupResponse(
  ...
  return {
    ...
    body: {
      ...
      // REQ-HAC-05 (#260): `roomId` es null en las filas nuevas (se asigna al check-in); lo
      // vendido es `roomType`. `roomId` se conserva para el replay de grupos viejos y los lectores.
      reservations: reservations.map((r) => ({
        id: r.id, roomId: r.roomId ?? null, roomType: r.roomType ?? null,

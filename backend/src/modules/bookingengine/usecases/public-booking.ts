...
//     con `needsCrib:true`. Cualquiera de las dos fuerza la key canónica en las keys (una sola
//     vez, sin duplicar) y se cobra SU precio real. Como la reserva nace sin unidad (REQ-HAC-05),
//     la línea se resuelve contra la UNIÓN de las unidades vendibles del tipo — hay cuna si
//     alguna la ofrece. `needsCrib`/`cribCount` (1/0) se persisten como espejo EXACTO de esa
//     línea resuelta: `needsCrib === (roomAmenities tiene una línea cuna)`, decidido después de
//     resolverla (los lee el PMS/housekeeping/correos).
...
export async function createPublicBookingDirect(
  ...
  // decide es si alguna unidad vendible del TIPO la ofrece (REQ-HAC-05: la reserva nace sin
  // unidad) — se resuelve más abajo, DESPUÉS de `resolveRoomAmenityLines` contra la unión del
  // tipo (`needsCrib` definitivo = quedó la línea).
  ...
  // #292/#341 — `needsCrib` definitivo: refleja lo que quedó resuelto contra la unión del tipo
  // (REQ-HAC-05: sin unidad), no el body. Es true si y sólo si la línea cuna quedó en el snapshot
  // (`hasCribLine`); así `needsCrib === (roomAmenities tiene una línea cuna)` siempre, y
  // `cribCount` es su espejo 1/0. Si se pidió y NINGUNA unidad vendible del tipo la ofrece, se
  // crea sin cuna con un warn claro.
  ...
  try {
    await orm.transaction(async (tx: any) => {
      ...
      reservation = await tx.create('Reservations', {
        ...
        // resueltos arriba contra la unión del tipo (REQ-HAC-05, sin unidad); acá solo persisten
        // como espejo EXACTO de la línea de cuna en `roomAmenities` (los leen PMS, housekeeping
        // y correos).
        needsCrib, cribCount,

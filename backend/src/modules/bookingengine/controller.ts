...
// #360 — Regímenes de alimentación, mismo patrón que upsells arriba (sub-dominio, sin service,
// catálogo abierto por hotel con CRUD completo).
...
export class BookingengineController {
   ...
   *  `bookingConfig` (opcional) es para marcar `mealPlansSeeded` tras sembrar los defaults (#360). */
  private assertMealPlansDeps(): mealPlansCrud.MealPlansCrudDeps {
    ...
    return { mealPlans: this.mealPlanRepo, userRepo: this.userRepoForUpsells, auth: this.authImpl, bookingConfig: this.bookingConfigRepo }
  }
  ...
  // ─── Regímenes de alimentación admin (#360) ──────────────────────────────
  // Catálogo abierto por hotel: list (siembra los 4 defaults una sola vez) + create/update/delete
  // por id. Mismo shape de respuesta que upsells.

  /** GET /api/meal-plans — regímenes del hotel del admin (ordenados por sortOrder, createdAt). */
  ...
  /** POST /api/meal-plans — alta de régimen (`code` lo genera el usecase a partir del name). */
  ...
  /** PUT /api/meal-plans/:id — edición (partial: name, description, priceMode, price, active, sortOrder). */

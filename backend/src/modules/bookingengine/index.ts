  ...
  return createModule({
    ...
    create({ logger, orm, cache, router, auth }: { logger: any; orm: any; cache: any; router: any; auth?: any }) {
      ...
      if (auth) {
        ...
        // #360 — Regímenes de alimentación admin (catálogo abierto, CRUD por id). El permiso
        // `mealplans` sigue siendo view/edit (ACTIONS['mealplans'] en shared/permissions.ts, sin
        // create/delete): alta y baja van bajo `edit`.
        ...
        router.post('/api/meal-plans', mealPlanGuard('edit'), (req: any) => controller.createMealPlan(req))
        router.put('/api/meal-plans/:id', mealPlanGuard('edit'), (req: any) => controller.updateMealPlan(req))
        router.delete('/api/meal-plans/:id', mealPlanGuard('edit'), (req: any) => controller.destroyMealPlan(req))
      }

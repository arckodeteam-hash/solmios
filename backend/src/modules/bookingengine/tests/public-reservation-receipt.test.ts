...
describe('getPublicReceiptPdf — recibo de pago público (#270)', () => {
  ...
  it('token válido → 200 application/pdf con el recibo completo', async () => {
    ...
    // Líneas del desglose.
    expect(html).toContain('Alojamiento · Vista Mar (101) · Doble')
  ...
  it('grupo: líder + 2 hermanas → 3 líneas de habitación y el total del grupo', async () => {
    ...
    expect(res.status).toBe(200)
    const html = res.body.toString()
    expect(html.match(/Alojamiento · Hab\. /g)?.length).toBe(3)
    expect(html).toContain('Alojamiento · Hab. 101 · Doble')
    expect(html).toContain('Alojamiento · Hab. 102 · Doble')
    expect(html).toContain('Alojamiento · Hab. 201 · Suite')

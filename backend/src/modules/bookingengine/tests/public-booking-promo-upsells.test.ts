...
describe('createPublicBookingDirect — F2 2.5 promo + upsells + atomic uses', () => {
  it('sin promo ni upsells → totalBreakdown con subtotal=room×nights', async () => {
    ...
    expect(b).toEqual({ subtotal: 200, promoDiscount: 0, upsellsTotal: 0, childAmenitiesTotal: 0, taxes: 36, taxBreakdown: [{ name: 'ITBIS', rate: 18, amount: 36 }], total: 236 })
  })
  ...
  it('promo válido percent → descuento aplicado, uses incrementado, total correcto', async () => {
    ...
    expect(res.body.totalBreakdown).toEqual({
      subtotal: 200, promoDiscount: 20, upsellsTotal: 0, childAmenitiesTotal: 0, taxes: 32.4, taxBreakdown: [{ name: 'ITBIS', rate: 18, amount: 32.4 }], total: 212.4,
    })
  ...
  it('promo fixed → descuento = min(value, subtotal)', async () => {
    ...
    expect(res.body.totalBreakdown).toEqual({
      subtotal: 200, promoDiscount: 50, upsellsTotal: 0, childAmenitiesTotal: 0, taxes: 0, taxBreakdown: [], total: 150,
    })
  ...
  it('upsells → upsellsTotal se suma al subtotal y al total', async () => {
    ...
    expect(res.body.totalBreakdown).toEqual({
      subtotal: 260, promoDiscount: 0, upsellsTotal: 60, childAmenitiesTotal: 0, taxes: 26, taxBreakdown: [{ name: 'IVA', rate: 10, amount: 26 }], total: 286,
    })
  })
  ...
  it('upsell inactivo/inexistente → se ignora (no rompe el flujo)', async () => {
    ...
    expect(res.body.totalBreakdown).toEqual({
      subtotal: 215, promoDiscount: 0, upsellsTotal: 15, childAmenitiesTotal: 0, taxes: 0, taxBreakdown: [], total: 215,
    })
  })
  ...
  it('sin extraDeps (compat F0 0.16) → no procesa promo, persiste el string sin validar', async () => {
    ...
    expect(res.body.totalBreakdown).toEqual({
      subtotal: 200, promoDiscount: 0, upsellsTotal: 0, childAmenitiesTotal: 0, taxes: 0, taxBreakdown: [], total: 200,
    })
  })
})

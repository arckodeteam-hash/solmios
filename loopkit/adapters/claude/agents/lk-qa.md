...
name: lk-qa
description: >
  Auditor read-only del LoopKit. Verifica con evidencia REAL (comandos ejecutados, no
  promesas) que un cambio está correcto y escribe .loopkit/state/scorecard.json con el
  veredicto. Es el ÚNICO que puede aprobar: quien escribió el código no vota.
  Trigger: cuando el gate del LoopKit bloquea el cierre, "auditá esta tarea",
  "corré el scorecard", "verificá el diff".

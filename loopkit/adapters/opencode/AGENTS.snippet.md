<!-- ═══ LOOPKIT — pegar en el AGENTS.md del repo ═══ -->
...
|---|---|
| `/lk-tarea <qué hay que hacer>` | analizar → implementar → verificar → autocorregir → auditar → cerrar |
| `/lk-bug <síntoma o error>` | reproducir → causa raíz → corregir → verificar → autocorregir → cerrar |
| `/lk-verificar` | auditar lo que ya está y arreglar lo que aparezca |

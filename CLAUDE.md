...
- **canales-admin-gestion-real** (`openspec/changes/canales-admin-gestion-real/`, GitHub epic #121, issues #122-#128): ✅ COMPLETO y en producción (2026-09-10, PR #184 + `49386c4d`; epic y #122-#128 cerrados). ⚠️ `planExpiresAt` está **VACÍO y así queda mientras prod apunte a `staging.channex.io`**: esa cuenta **no tiene plan ni vencimiento** (verificado 2026-09-10 en Organization → Billing: "Usage Details: Nothing here yet"). El 2026-09-10 se había cargado `2026-09-09` confundiéndolo con el trial del hotel en SolmiOS (`subscriptions.trialEndsAt`) y la tarjeta mostró en rojo "Plan de Channex vencido" sin fundamento; se vació. No volver a cargar una fecha ahí. **Channex NO expone el vencimiento por API**: el campo vuelve a importar cuando se migre a la cuenta de producción (`app.channex.io`) con plan pago — se carga desde la tarjeta y la alerta ya está verificada. **Webhook de reservas REGISTRADO** (verificado 2026-09-12 vía `GET /api/admin/channex-webhook`: callback `https://solmios.com/api/channels/channex/webhook?api_key=…`, eventos `booking_new;booking_modification;booking_cancellation`, a nivel cuenta — `propertyId: null`); el cron de 15 min queda de respaldo. Las 5 properties huérfanas de la cuenta staging se **borraron el 2026-09-12** (`DELETE /api/v1/properties/:id`): quedaron 9 properties = 9 hoteles. La certificación PMS tiene su guion operativo en `CHANNEX-CERTIFICACION-CHECKLIST.md` y cada punto resuelto en `CHANNEX-CERTIFICACION-RESPUESTAS.md` (#38). **Registro de Channex (#347, 2026-09-12)**: todo lo que le pasa al transporte (espera por rate limit, 429, 5xx, timeout, reintentos agotados, 4xx) y al webhook (ingestada / plan B / rechazada) queda en `sync_log` vía `canales/usecases/channex-trail.ts` (hotel por `property_id`, o `platform`) y se ve en `/admin/channex-queue?tab=registro` (`GET /api/admin/channex/log`, paginado). El cron del feed ya NO escribe la fila "0 reservas" (había 10.441; respaldadas en `/root/backups/sync_log-cron-vacio-2026-09-12.tsv` del server y borradas). Webhook de Channex: el callback DEBE registrarse con `send_data: true` (#342) — con `false` Channex omite `payload` y no llega `revision_id`. `/admin/channels` dejó de ser una tabla con `<select>` libre: `channel_requests` tiene ciclo de vida con transiciones (`pending → scheduled → in_progress → waiting_hotel → connected|rejected`, cualquier otra = 409), cita obligatoria con fecha/medio/contacto, historial en `channel_request_activities`, avisos (correo al soporte + campanita al super-admin al entrar el pedido; 3 plantillas de plataforma al hotel al agendar/conectar/rechazar) y cron diario 08:00 de citas de hoy/vencidas. La tarjeta de cuenta Channex muestra webhook, properties huérfanas y vencimiento del plan. En prod ya corrieron `RUN_MIGRATE=1` y `seed-platform-email-templates.ts --refresh` (20 plantillas).
...
- **reserva-web-pago-confirmado** (GitHub epic #243, issues #244-#250): ✅ COMPLETO y en producción (2026-09-12). REQ-RWP-01 tabla `payment_attempts` (dueña `payment-gateways`, guarda TODO outcome de la pasarela incluidos `failed`/`expired`; webhook Stripe `payment_intent.payment_failed` en `services/payment-gateway/stripe-gateway.ts`) · REQ-RWP-02 bloque "Pasarela de pago" en el detalle de la reserva (`reservas/usecases/detail.ts` lee los intentos por puerto opcional, connector `reservas-payment-gateways.ts`; UI `ReservationModal.vue`) · REQ-RWP-03 aviso al hotel por reserva web/OTA y por pago confirmado — campanita a todo usuario con `reservations:view`, correo a `hotels.email` y push (`shared/usecases/notify-reservation-received.ts`; connectors `bookingengine-notificaciones.ts` y `canales-notificaciones.ts` vía socket `onOtaBookingIngested`) · REQ-RWP-04 listado con estado de pago por fila (`paymentState` calculado en `reservas/usecases/crud.ts`, misma fórmula que el detalle), KPI Cobradas y `source:'web'` para el motor (`channel` sigue `direct`) + backfill `scripts/backfill-reservation-source-web.ts` (fila en la tabla de scripts) · REQ-RWP-05 vencimiento de reservas web sin pago (cron, `pendingPaymentTtlHours` por hotel, default 24, 0 = nunca; libera la habitación con `reason:'payment_timeout'`) · REQ-RWP-06 `POST /api/reservas/:id/mark-paid` (pago manual con evidencia, guard `billing:create`). Deploy: `RUN_MIGRATE=1` creó `payment_attempts` y `bun run migrate` corrió el backfill (auto-deploy); verificado en prod 2026-09-12: el hotel demo tiene 22 reservas `source='web'`. Evidencia del recorrido en prod: `docs/evidencia/reserva-web-pago-confirmado/`. Spec: los REQ-RWP-0x viven en `openspec/specs/reservas/spec.md` (no hay carpeta propia en `openspec/changes/`). **Deuda documentada (no hecha)**: unificar los tres webhooks de Stripe (`/api/public/webhook/stripe`, `/api/webhooks/stripe`, `/api/stripe/webhook`) en uno con un solo asiento · `payment_attempts` para POS y links del panel (`source: 'pos' | 'payment_request'` reservados) · aviso al hotel por WhatsApp.
- **habitacion-asignada-al-checkin** (GitHub epic #255, issues #256-#263, REQ-HAC-01..07 en `openspec/specs/reservas/spec.md` §906-1123 — no hay carpeta propia en `openspec/changes/`): ✅ COMPLETO y en producción (2026-09-13, cierre #263). **`roomType` es lo vendido; `roomId` es dónde duerme y puede ser nulo hasta el check-in.** La disponibilidad se decide SIEMPRE por `availableOfType` (`shared/usecases/type-availability.ts`: cuenta también las reservas sin asignar); el solape físico solo al asignar (`assign-room.ts`). El código TTLock se genera al asignar, no al pagar (`connectors/reservas-ttlock.ts`, `onRoomAssigned`). Web y OTA nacen sin habitación (arregla el overbooking de OTAs: dos reservas del mismo tipo y fechas entran las dos y se asignan después); `POST /api/reservas/:id/assign-room {roomId, allowTypeChange?}` / `DELETE` / `GET …/assignable-rooms`; el check-in exige habitación y la asigna en el mismo paso (`RoomAssignModal.vue`); planning con banda "Sin asignar" (drag & drop) y badge en listado/modal; pase pre-llegada, auto-mensajes, housekeeping y reports toleran `roomId` null. Deploy: `RUN_MIGRATE=1` (columnas nuevas) + `bun run migrate` (`scripts/relax-reservations-roomid.ts` y `scripts/backfill-reservation-room-type.ts`, filas en la tabla de abajo) — los dos los corre el auto-deploy; verificado en prod por API el 2026-09-13 (sin SSH: 0 reservas del hotel demo con `roomType` vacío e INSERT de una reserva web con `roomId: null` = `NOT NULL` relajado; las consultas SQL quedan escritas en el README). Gates + recorrido de punta a punta en prod con capturas: `docs/evidencia/habitacion-asignada-al-checkin/` (`gates.md`, `README.md`, `recorrido-prod.mjs`). **Nota de changelog para hoteles con TTLock**: el código llega al asignar la habitación (planning, check-in o auto-asignación), no al reservar/pagar; si quieren el código la víspera, `autoAssignBeforeArrivalHours` en la config del motor (`bookingengine/usecases/config.ts`, 0-168 h, default 0 = apagado; lo consume `shared/usecases/prearrival-pass-cron.ts`). **Hallazgos del recorrido (no arreglados acá)**: la caché de 300 s del listado de reservas no se invalida con la ingesta de Channex (una OTA nueva tarda hasta 5 min en `/panel/reservas`; el planning la ve al instante) · `GET /api/reservas?search=` devuelve 0 en prod · la caché del listado de folios no se invalida con assign-room (el detalle sí) · reasignar con `allowTypeChange` cambia `roomType` (afecta reports por tipo) · las reservas de Channex quedan sin `guestId` hasta el check-in. **Deuda documentada (no hecha)**: entidad `room_types` con id propio (hoy `rooms.type` string, clave compartida con `room_type_capacity` y el mapeo de Channex) · overbooking controlado por tipo (vender N+1 a propósito) · preferencias del huésped (planta, vista) como criterio de sugerencia de habitación.
...
| `scripts/relax-reservations-roomid.ts` (lo llama `migrate-db.ts`, NO se corre aparte) | Quita el `NOT NULL` de `reservations.roomId` (#256/#258, REQ-HAC-01): la habitación se asigna al check-in, lo vendido es `roomType`. PG: `ALTER COLUMN roomid DROP NOT NULL` (mira `information_schema.columns.is_nullable` antes); SQLite: recrea la tabla sin el NOT NULL copiando por nombre desde `pragma_table_info` y re-creando índices, en transacción, con `PRAGMA foreign_keys=OFF` fuera del BEGIN. Molde de `drop-users-role-check.ts` pero sobre `db.query/db.run` del DbAdapter. | ✅ (`changed:false` si el DDL ya no tiene el NOT NULL; test en `reservas/tests/relax-roomid-backfill-roomtype.e2e.test.ts`) |
| `scripts/backfill-reservation-room-type.ts` (lo llama `migrate-db.ts`, NO se corre aparte) | Rellena `reservations.roomType` = `rooms.type` (#258, REQ-HAC-01) en las filas con `roomType` NULL/'' y `roomId` existente en `rooms`; sin esto las reservas anteriores a #258 quedan "sin tipo vendido" y el chequeo de `type_mismatch`/asignables no sirve. Todo en SQL, sin nombres de columna en JS (Postgres pliega a minúsculas). | ✅ (segunda corrida: 0 filas; mismo test) |
| `scripts/backfill-crib-room-amenity.ts` (lo llama `migrate-db.ts`; también corre suelto con `DB_PATH`/`DATABASE_URL`) | Transición de datos de #292: la cuna dejó de ser el toggle global `configuration.child_policy.cribAvailable` y pasó a ser la amenidad `custom:cuna` de CADA habitación (`room_amenities`). A todo hotel con `cribAvailable: true` le crea esa fila ("Cuna", activa, precio 0 — o `cribPrice` si la config lo tuviera) en las habitaciones que no tengan ya una cuna (reconocida por `shared/usecases/crib-amenity.ts`: `custom:cuna`/`custom:crib`/`custom:berco` o nombre con "cuna"/"crib"/"berço"; una desactivada NO se reactiva). Sin esto, el motor dejaba de preguntar "¿Necesita cuna?" en prod hasta cargarla a mano habitación por habitación. No toca `configuration` ni el CSV `rooms.amenities`. | ✅ (0 filas la segunda corrida; test en `bookingengine/tests/backfill-crib-room-amenity.test.ts`) |
...
| `scripts/seed-platform-email-templates.ts` | Inserta las 20 plantillas de correo de PLATAFORMA (`platform_email_templates`: welcome, trial_* — incl. `trial_extended`, la que encola `POST /api/admin/subscriptions/:hotelId/extend-trial` (#146) —, renewal_*, payment_*, suspended, reactivated, canceled, más las 6 de la secuencia de activación/rescate `activation_*`, `trial_offer`, `trial_rescue_*` que manda `shared/usecases/activation-sequence-cron.ts` (#149/#150), más las 3 del pedido de conexión de OTA — `channel_request_scheduled/connected/rejected`, cuyo texto vive en `src/shared/usecases/channel-request-email-templates.ts` porque lo comparte un test de render). Insert-only por defecto. Con `--refresh` **reemplaza** subject/body/variables de las 20 con el texto del script (conserva `isActive`). El nombre de la plataforma NO va escrito: es `{platform_name}` y lo resuelve el envío desde `configuration('plataforma')` (`shared/utils/platform-identity.ts`), igual que `{support_email}`/`{support_phone}`. **Correr en prod con `--refresh` tras el deploy 2026-09-09**: las filas viejas tienen "SolmiOS" hardcodeado. **NO confundir con las plantillas de HOTEL/huésped**: `reservation_new_staff` (aviso al staff por reserva web / pago recibido, #267), `reservation_new_ota_staff` (aviso al staff por reserva entrada desde una OTA: nombra al canal en `{channel_name}` y NO afirma estado del pago, porque la ingestión de Channex no recibe dato de cobro) y `reservation_received_unpaid` (huésped sin pasarela de pago) viven en código en `src/services/notification-defaults.ts` (es/en/pt, `{platform_name}` resuelto por `platformIdentity`), NO en `platform_email_templates`: no requieren seed ni `--refresh`, y el hotel puede pisarlas desde `auto_messages` — el evento tiene que estar en `NOTIFICATION_EVENTS` de `marketing/validators/schema.ts` Y en el `<select>` de `pages/auto-messages/index.vue`, si no el override no se puede guardar (400). | ✅ (COUNT por evento; `--refresh` = UPDATE por evento) |
...
- **`payment_attempts` es rastro, no saldo — `payments` sigue siendo la única fuente de verdad del dinero**
  (#243/#244): un intento `failed`/`expired`/`succeeded` de la pasarela NUNCA suma ni resta al pagado;
  `paidForReservation` (`shared/usecases/reservation-paid.ts`) y el listado sólo miran `payments`/folio/factura,
  y `payment_events` (id `provider:eventId`) sigue siendo la barrera de idempotencia.

...
- **canales-admin-gestion-real** (`openspec/changes/canales-admin-gestion-real/`, GitHub epic #121, issues #122-#128): ✅ código completo y en producción (2026-09-10, PR #184 + `49386c4d`; epic y #122-#127 cerrados, **#128 sigue abierto**). ⚠️ `planExpiresAt` está **VACÍO** a propósito: el 2026-09-10 se cargó ahí `2026-09-09` confundiéndolo con el trial del hotel en SolmiOS (`subscriptions.trialEndsAt`) y la tarjeta mostró en rojo "Plan de Channex vencido" sin fundamento; se vació. **Channex NO expone el vencimiento de su plan por API**: la fecha sale del dashboard (staging.channex.io) y la carga una persona desde la tarjeta — hasta entonces no hay alerta y #128 queda abierto solo por eso. **Pendiente de OPERACIÓN, no de código**: el webhook de reservas de Channex no está registrado (las reservas de OTAs entran solo por cron, con retraso — la tarjeta lo avisa en ámbar) y hay 5 properties huérfanas en la cuenta. `/admin/channels` dejó de ser una tabla con `<select>` libre: `channel_requests` tiene ciclo de vida con transiciones (`pending → scheduled → in_progress → waiting_hotel → connected|rejected`, cualquier otra = 409), cita obligatoria con fecha/medio/contacto, historial en `channel_request_activities`, avisos (correo al soporte + campanita al super-admin al entrar el pedido; 3 plantillas de plataforma al hotel al agendar/conectar/rechazar) y cron diario 08:00 de citas de hoy/vencidas. La tarjeta de cuenta Channex muestra webhook, properties huérfanas y vencimiento del plan. En prod ya corrieron `RUN_MIGRATE=1` y `seed-platform-email-templates.ts --refresh` (20 plantillas).
- **admin-soporte-real** (`openspec/changes/admin-soporte-real/`, GitHub epic #120, issues #129-#136): 6 de 8 en prod (#129-#134: solicitante/hotel/agente resueltos por el servidor, conversación con autor real, "Entrar como", el hotel ve quién lo atiende). Quedan #135 (notificación al hotel con el nombre del agente) y #136 (verificación e2e), tomados por AUTOWORK. Spec REQ-SOP-01..07.

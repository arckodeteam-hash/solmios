...
- **restaurante-pos-real** (GitHub epic #202, issues #203-#217): auditoría 2026-09-11 del módulo Restaurante (salón, comanda, KDS, cobrar, caja, carta) sobre prod + código. El circuito completo funciona y la plata va a `payments`/caja del restaurante/folio; lo que falta es de POS de uso diario: **vistas en pestañas** (`PillTabs` compartido, #203 — Carta apila 5 secciones; Caja ya usa pill-tabs y es la referencia), permisos coherentes (#205: `kitchen` entra por URL a salón/caja/cobrar; el mozo no puede quitar una línea), **bug de dinero** #206 (`order-lines.ts:287` pierde el `priceDelta` de modificadores al cambiar cantidad), ~~anulaciones con motivo + auditoría (#207)~~ ✅ (2026-09-11: `voided` en `restaurant_order_items` con `voidReason/voidedBy/voidedAt`; `POST /orders/:id/items/:lineId/void {reason}` y `/cancel {reason}` obligatorio; `DELETE` de una línea enviada → 409; el KDS ya no transiciona a `cancelled` — "Cancelar" abre `VoidReasonModal` con los motivos de `configuration('restaurant_void_reasons')` (`GET/PUT /api/restaurant/void-reasons`, sin pestaña de UI todavía); connector `restaurante-auditlog.ts` registra `restaurant.line.voided|order.cancelled|order.refunded`. `isLineActive()` en `order-totals.ts` es el ÚNICO lugar que decide qué línea cuenta — no comparar `status` a mano), cargo a habitación sin tipear UUID (#209), y lo que no existe: tiempo real, cierre del día, dividir cuenta, descuentos, impresión. Orden y dependencias en el epic.

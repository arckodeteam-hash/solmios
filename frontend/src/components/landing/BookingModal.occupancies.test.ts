...
describe('BookingModal — composer de huéspedes (adultos+niños+edades)', () => {
  ...
  it('#360 el radio lista SOLO el catálogo activo, con el name de cada fila (custom incluido) y la primera marcada', async () => {
    vi.mocked(BookingService.getMealPlans).mockResolvedValue([
      ...
      { code: 'brunch_premium', name: 'Brunch premium', priceMode: 'per_person_per_night', price: 25 },
    ])
    await open()

    const radios = Array.from(document.body.querySelectorAll<HTMLInputElement>('[role="radiogroup"] input[type="radio"]'))
    expect(radios.map((r) => r.value)).toEqual(['room_only', 'breakfast', 'brunch_premium'])
    expect(radios[0]!.checked).toBe(true)
    for (const r of radios) expect(r.disabled).toBe(false)
    const text = document.body.textContent ?? ''
    // El `name` configurado por el hotel manda sobre la etiqueta legacy del código.
    expect(text).toContain('Solo alojamiento')
    expect(text).toContain('Desayuno buffet')
    expect(text).not.toContain('Desayuno incluido')
    expect(text).toContain('Brunch premium')
    expect(text).not.toContain('brunch_premium')
    expect(text).not.toContain('Todo incluido')
    expect(text).not.toContain('Este hotel no ofrece este régimen')
  })

  it('régimen incluido en la tarifa: primera opción ya marcada con "Incluido"; cambiar a otra la marca', async () => {
    vi.mocked(BookingService.getMealPlans).mockResolvedValue([
      { code: 'breakfast', name: 'Desayuno incluido', priceMode: 'included', price: 0 },
      { code: 'half_board', name: 'Desayuno y cena', priceMode: 'per_person_per_night', price: 20 },
    ])
    await open()
    ...
    const label = labels.find((l) => l.textContent?.includes('Desayuno incluido'))!
    expect(label.textContent).toContain('Incluido')
    expect(label.className).toContain('bg-navy')
    const other = labels.find((l) => l.textContent?.includes('Desayuno y cena'))!
    expect(other.className).not.toContain('bg-navy')

    const halfBoard = document.body.querySelector<HTMLInputElement>('input[value="half_board"]')!
    halfBoard.checked = true
    halfBoard.dispatchEvent(new Event('change'))
    await flushPromises()
    expect(other.className).toContain('bg-navy')

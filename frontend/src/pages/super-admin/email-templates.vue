<template>
  <div>
    ...
    <div class="mb-6">
      ...
      <p class="text-sm text-text-muted mt-0.5">Editá el contenido y activá o desactivá los emails automáticos de la plataforma (bienvenida, trial, pagos, renovaciones, suspensión, cancelación)</p>
    </div>
    ...
    <SectionCard title="Eventos de la plataforma" :subtitle="`${orderedTemplates.length} de ${EVENT_ORDER.length} plantilla(s)`" body-class="p-0">
      ...
      <div v-if="loading" class="space-y-3 p-5">
        <div v-for="i in EVENT_ORDER.length" :key="i" class="h-12 animate-pulse rounded-xl bg-surface"></div>
      </div>
...
// Los 10 eventos son fijos: no se crean ni se borran filas, solo se editan.
// Los labels viven en PlatformEmails.service.ts (compartidos con la tarjeta de /admin/settings).
const EVENT_ORDER: PlatformEmailEvent[] = [
  ...
  'subscription_renewal_auto', 'subscription_renewal_manual', 'subscription_suspended', 'subscription_reactivated',
]
...
// Si la ruta trae ?event=<evento> (desde la tarjeta de /admin/settings) se abre directo el editor.
onMounted(async () => {
  await load()
  const wanted = typeof route.query.event === 'string' ? route.query.event : null
  if (!wanted) return
  const tpl = templates.value.find(t => t.event === wanted)
  if (tpl) openEdit(tpl)
})
</script>

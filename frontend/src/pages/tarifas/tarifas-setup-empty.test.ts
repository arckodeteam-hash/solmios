...
vi.mock('@/services/Hotel.service', () => ({
  HotelService: {
    ...
    // Mock incompleto post-merge (Tarea 2, QA 2026-08-20): sin este método, onMounted() tira
    // sincrónicamente ("pricingMode is not a function") ANTES de llegar a `loadRates()` — la
    // matriz nunca se reconstruye con `ratesData` y el estado vacío queda pegado aunque haya
    // tarifas. `pricingMode` es real (`Hotel.service.ts`), el mock solo estaba desactualizado.

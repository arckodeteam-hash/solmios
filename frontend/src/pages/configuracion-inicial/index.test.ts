...
// re-testea acá: los 8 steps se stubean por completo para que este archivo pruebe SOLO la
...
describe('Centro de configuración — barra de pasos', () => {
  it('muestra los 8 pasos en el orden que devuelve el backend', async () => {
    statusImpl = async () => ({ completed: false, doneCount: 1, totalCount: 8, steps: STEPS_8 })
    const wrapper = await mountShell()

    const buttons = wrapper.findAll('nav button')
    expect(buttons.length).toBe(8)
    // SHORT_LABEL de cada key, en orden.
    const labels = ['Bienvenida', 'Identidad', 'Contacto', 'Ubicación', 'Políticas', 'Habitaciones', 'Tarifas', 'Canales']
  ...
  it('el % mostrado cuenta SOLO los pasos requeridos (mismo criterio que ProfileProgressBar.vue)', async () => {
    // STEPS_8 tiene 5 requeridos (bienvenida, identidad, ubicacion, politicas, rooms), 1 hecho
    // (bienvenida) = 20% — NO 60%, que daría doneCount/totalCount del backend (6/8) si se
    // contaran los opcionales.
    statusImpl = async () => ({ completed: false, doneCount: 6, totalCount: 8, steps: STEPS_8 })
    const wrapper = await mountShell()
  ...
  it('cuando todo lo requerido está completo, muestra el cierre en vez del contenido del paso 0', async () => {
    statusImpl = async () => ({
      completed: true, doneCount: 8, totalCount: 8,
      steps: STEPS_8.map((s) => ({ ...s, done: true })),
    })
    const wrapper = await mountShell()

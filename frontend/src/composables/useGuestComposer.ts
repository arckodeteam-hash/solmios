...
/** MR-03 (#268) / #361 — una opción del radio de régimen de una tarjeta: UNA por fila del
 *  catálogo público (`store.mealPlans`), en el orden recibido y con el `name` tal cual lo
 *  configuró el hotel. Nada fijo: ni "Solo alojamiento" implícito, ni códigos deshabilitados —
 *  lo que el backend no manda (switch apagado / inactivo) NO existe para el widget. `total` =
 *  importe para la composición actual de la tarjeta (0 si `included`). `available` queda `true`
 *  siempre (toda fila recibida es reservable) — se conserva por compat con la UI. */
...
export function useGuestComposer() {
   ...
   *  (cubre un click programático o un estado viejo). */
   ...
   *  composición actual (`price × personas × noches`, misma fórmula que el backend). #361: UNA
   *  por fila de `store.mealPlans`, en el orden recibido, con el nombre del catálogo; el backend
   *  ya filtró (switch + activos), así que acá no se agrega ni se deshabilita nada. `[]` = el
   *  hotel no ofrece regímenes (la UI no renderiza la sección). */
  function mealPlanOptions(rt: RoomTypeRate): MealPlanOption[] {
    ...
    return store.mealPlans.map((m): MealPlanOption => {
      const unitPrice = Number(m.price) || 0
      return {
        code: m.code, name: String(m.name ?? '').trim() || m.code,
        priceMode: m.priceMode, unitPrice, available: true,
        total: computeMealPlanTotal(m.priceMode, unitPrice, persons, nights),
      }
    })
  }

  /** Importe del régimen elegido para la composición actual (para el "+ $X" de la tarjeta). 0 sin
   *  catálogo, con `included` o si el código dejó de estar en el catálogo. Cambiar la selección
   *  REEMPLAZA el importe (nunca se acumulan dos regímenes en una tarjeta). */
  function composedMealPlanTotal(rt: RoomTypeRate): number {
    const code = mealPlanCode(rt)
    if (code === null) return 0
    const option = mealPlanOptions(rt).find((o) => o.code === code)
    return option ? option.total : 0
  }
  ...
  async function addComposedRoom(rt: RoomTypeRate): Promise<void> {
    ...
    await store.addToCart(rt, {
      ...
      ...(mealPlanToSend !== null ? { mealPlan: mealPlanToSend } : {}),
    })
  ...
  function editCartLine(line: CartLine): boolean {
    ...
    composerState()[rt.id] = {
      ...
      // perdía en silencio (y con él su cobro). Sin snapshot se omite, igual que los otros opcionales.

...
export const useBookingStore = defineStore('booking-widget', () => {
  ...
  function cartLineKeyForComposition(
    ...
    childAmenityIds: string[] = [], roomAmenityKeys: string[] = [], mealPlan: string | null = null,
  ): string {
  ...
  /** Subtotal room(s)+upsells+amenidades infantiles+amenidades de habitación+régimen ANTES de
   *  promo e impuestos — misma cuenta que `totalBreakdown.subtotal` del backend. Promo se aplica
   *  sobre este monto. */
  ...
  async function addToCart(
    ...
    const key = isComposition
      ? cartLineKeyForComposition(
        ...
        lineChildAmenities.map((a) => a.id), lineRoomAmenities.map((a) => a.key), lineMealPlan?.code ?? null,
      )

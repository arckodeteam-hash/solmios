 ...
 *  mostrarla en el resumen/pago (espejo de `UpsellLine`, más `roomName`/`lineKey` porque acá la
 *  selección es POR HABITACIÓN). `total` = `price × quantity` (quantity = unidades de la línea). */
 ...
 *  `RoomAmenityLine`, pero UNA fila por línea, no por ítem). Una fila por línea con snapshot
 *  (#361: cualquier código del catálogo visible, incluido `room_only` si el hotel lo ofrece);
 *  las `included` van con `total` 0 para que el desglose diga "incluido". `total` =
 *  `mealPlan.total × quantity`. `name` = nombre del catálogo al agregar (la etiqueta). */
...
export interface CartLine {
   ...
   *  `needsCrib` (a diferencia de `selectedUpsells`, global al carrito), SIN gateo por niños (una
   *  cama extra se pide para cualquier composición). SNAPSHOT del catálogo por tipo
   *  (`roomAmenitiesFor(roomType)`) tomado al agregar: nombre y precio quedan fijos para el
   *  resumen aunque el catálogo cambie después. Al backend viaja solo la `key`
   ...
   *  catálogo (`store.mealPlans`) tomado al agregar: código, NOMBRE (#361), modo, precio unitario,
   *  personas que pagan y el importe por unidad ya calculado. Al backend viaja solo el código
   *  (`mealPlan`) — él recalcula contra su catálogo visible. `undefined` = sin régimen (catálogo
   *  vacío o código que ya no está). #361: se guarda para CUALQUIER código del catálogo, incluido
   *  `room_only` cuando es una fila. Importe de la línea = `mealPlan.total × quantity`. */
...
export interface ComposerState {
  ...
  // ausente = la primera opción del catálogo (#361: sin códigos fijos ni `room_only` implícito).
  // Se conserva al cambiar adultos/niños — solo cambia el importe. Leer vía `mealPlanCode(rt)`.
...
export const useBookingStore = defineStore('booking-widget', () => {
  ...
  function cartLineKeyForComposition(
    ...
    // cuna: misma composición con distintas amenidades = habitaciones DISTINTAS (keys ordenadas
    // para que el orden en que se tildaron no genere dos líneas para la misma elección). El
    // segmento `|ra...` se agrega SOLO cuando hay alguna, para que las keys de líneas sin
    ...
    // DISTINTAS (una con desayuno y otra sin no se funden en "×2"). Segmento solo si hay
    // snapshot; #361: CUALQUIER código del catálogo cuenta (incluido `room_only` como fila) —
    // sin esto una línea "solo alojamiento" y una sin régimen se fundirían.
  ...
  // el precio mínimo del tipo. Precargado en `search()` junto a mealPlans/upsells por el mismo
  // motivo: el composer de cada tarjeta lo necesita ANTES de "Agregar esta habitación". Nombres y
  // precios vienen SIEMPRE del backend — nunca hay lista ni precios en código. La selección NO
  // vive acá sino en cada `CartLine` (`CartLine.roomAmenities`), fallo silencioso → `{}`.
  // #292 — la cuna (`custom:cuna`, `CRIB_AMENITY_KEY`) es una amenidad más de este catálogo.
  ...
  /** MR-03 (#268) — régimen elegido, una fila por línea del carrito que lo tenga (#361: cualquier
   *  código del catálogo, `room_only` incluido si es una fila), para verlo por separado en el
   *  resumen/pago (espejo de `roomAmenityLines`). Las `included` entran con `total` 0 — el
   *  desglose las muestra como "incluido", no las oculta. */
  const mealPlanLines = computed<MealPlanLine[]>(() => {
    const lines: MealPlanLine[] = []
    for (const l of cart.value) {
      const mp = l.mealPlan
      if (!mp || !mp.priceMode) continue
      lines.push({
  ...
  async function search(): Promise<void> {
    ...
    // REQ-01 (#290) — catálogo de amenidades de habitación por tipo, mismo criterio que
    // mealPlans/upsells: en paralelo, fallo silencioso a `{}` (sin catálogo simplemente no se
    // ofrecen).
    ...
    try {
      const [res, mp, ups, ram] = await Promise.all([
   ...
   * `{code, name, priceMode, unitPrice, persons, total}` en la línea, con `total = price ×
   * (adultos + niños con plaza) × noches` (0 si `included`) — misma fórmula que el backend. #361:
   * cualquier código del catálogo visible vale (incluido `room_only` si es una fila); un código
   * que no está en el catálogo se ignora: la línea queda sin régimen.
   */
  async function addToCart(
    ...
    occupancy?: number | {
      ...
      roomAmenityKeys?: string[]
      // MR-03 (#268) — régimen de ESTA línea (código del catálogo visible; omitido = sin régimen).
      mealPlan?: string
    },
  ): Promise<void> {
    ...
    if (existing) {
      ...
      cart.value.push({
        ...
        // MR-03 (#268) — solo cuando el código resolvió contra el catálogo (#361: incluido
        // `room_only` si el hotel lo ofrece como fila).
  ...
  /** MR-03 (#268) — resuelve el código elegido contra el catálogo visible cargado
   *  (`store.mealPlans`). Devuelve el snapshot `{code, name, priceMode, unitPrice, persons, total}`
   *  o `null` para omitido/código que no está en el catálogo (el backend igual lo rechazaría con
   *  `meal_plan_unavailable` — acá se degrada a sin régimen, nunca se muestra un total con un
   *  régimen que después no se cobra). #361: `room_only` se resuelve como cualquier otra fila. */
  function resolveMealPlan(code: string | undefined, persons: number): CartLineMealPlan | null {
    ...
    return {
      code: found.code, name: String(found.name ?? '').trim() || found.code,
      priceMode: found.priceMode, unitPrice, persons,
      ...
      if (cart.value.length === 1 && cart.value[0]!.quantity === 1) {
        ...
        res = await BookingService.createBooking({
          ...
          ...(line.roomAmenities && line.roomAmenities.length > 0
            ...
            : {}),
          // MR-03 (#268) — solo el código: el backend recalcula contra su catálogo visible (#361:
          // `room_only` viaja también cuando es una fila elegida del catálogo).
        ...
        res = await BookingService.createBookingGroup({
          ...
          rooms: cart.value.map((l) => ({
            ...
            // REQ-01 (#290) — POR LÍNEA, igual que la cuna.

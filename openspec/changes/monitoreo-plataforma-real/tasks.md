...
- [ ] 1.1 `shared/observability/route-key.ts`: normalizar la ruta (UUID/ULID/números → `:id`).
...
- [ ] 1.2 `shared/observability/metrics.ts`: agregado en memoria por ruta (count, avg, p95, max,
      errores) con tope `MAX_ROUTES` y desborde a `otras`.
      **Aceptación**: test que supera el tope y verifica que el mapa no crece más.
- [ ] 1.3 Middleware `httpMetrics()` con la firma del framework `(req, next)`.
...
- [ ] 1.4 Registrar el middleware en `composition-root.ts`.
      **Aceptación**: `arckode analyze` en 0 violaciones; `bun test` verde.
- [ ] 1.5 `GET /api/admin/monitoring/api` (super_admin).
      **Aceptación**: test de ruta que verifica 403 para `merchant`.

## 2. Errores recientes (REQ-MON-02)

- [ ] 2.1 Modelo `error_logs` + registro en `composition-root`.
...
- [ ] 6.2 Eliminar las menciones a AWS S3, CloudFront y SendGrid: el proyecto no usa ninguno.
      **Aceptación**: `grep -i "s3\|cloudfront\|sendgrid" monitoring.vue` → vacío.
- [ ] 6.3 Estado global derivado de las mediciones; con el backend caído NO dice "operativo".
      **Aceptación**: prueba con la API cortada → la pantalla informa el fallo.
- [ ] 6.4 Botón "Refrescar" funcional y botón de backup con progreso y resultado.
      **Aceptación**: verificado en el navegador, no solo typecheck.
- [ ] 6.5 Sección de entregas de webhook y colas.
      **Aceptación**: se ve una entrega fallida con su código de respuesta.
- [ ] 6.6 Tipos en `Platform.service.ts` sin `any`.
...
- [ ] 7.5 Confirmar que un `merchant` autenticado recibe 403 en los cinco endpoints nuevos.

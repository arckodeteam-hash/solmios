...
- [ ] 1.1 `sales-leads/model.ts`: modelo `SalesProspects` (tabla `sales_prospects`) con `hotelId`,
...
- [ ] 1.2 `sales-leads/usecases/pipeline.ts`: `buildPipeline(deps)` con repos inyectados de
...
- [ ] 1.3 `GET /api/admin/sales-pipeline` en `sales-leads/index.ts` (append-only).
      **Aceptación**: test de ruta: `merchant` → 403; respuesta con `whatsappUrl` en E.164
      (`809-555-0000` → `https://wa.me/18095550000`).
- [ ] 1.4 `PUT /api/admin/sales-pipeline/:key` con `UpdateProspectSchema` (validateSchema), upsert,
...
- [ ] 2.1 `subscriptions` expone socket `onHotelSignedUp(hotel, owner, planId)`; connector
...
- [ ] 2.2 `sales-leads/usecases/emails.ts`: `buildSignupAlertEmail` con `wa.me` + texto prellenado +
      link a `/admin/leads-ventas`. Best-effort.
...
- [ ] 3.1 `POST /api/admin/subscriptions/:hotelId/extend-trial {days}` en `admin` (delega a
      ...
      **Aceptación**: test: vencido hace 5 días + 7 → `trialEndsAt = now+7d`, `status='trialing'`,
      ...
      fila en `audit_log`.
- [ ] 3.2 Plantilla `trial_extended` en `seed-platform-email-templates.ts` + `PlatformEmailEvent` +
...
- [ ] 4.1 `frontend/src/services/SalesPipeline.service.ts` + tipos en `types/`.
- [ ] 4.2 Reescribir `pages/super-admin/leads-ventas.vue`: bloque "Vencen hoy", filtros
      ...
      **Aceptación**: `bun run typecheck`; QA en navegador con la SQLite local (seed de 3 hoteles en
      etapas distintas): screenshot de la vista adjunto al issue; "Perdido" sin motivo no dispara
      petición (Network); `nextStepAt` ayer → aparece en "Vencen hoy".
- [ ] 4.3 Menú: "Leads de Ventas" pasa a llamarse "Pipeline de ventas" (`SuperAdminLayout.vue`, ruta
      igual).

### 5. WhatsApp en la landing (REQ-PIPE-07)

- [ ] 5.1 `GET /api/public/platform-contact` → `{ whatsappUrl | null }` desde
...
- [ ] 5.2 Botón flotante en `pages/landing/index.vue` solo si hay URL.
      **Aceptación**: screenshot con y sin número; sin número el DOM no tiene el botón.
